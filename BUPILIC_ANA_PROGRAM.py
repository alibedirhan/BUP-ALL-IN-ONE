...
# --- GLOBAL TK DISTANCE HOTFIX (GÜÇLENDIRILMIŞ) ---
...
def _sanitize_distance_value(v):
    ...
    try:
        ...
        # String kontrolü - tüm float formatlarını yakala
        if isinstance(v, str):
            # "200.0", "200.00", "200." gibi formatları yakala
            if re.match(r'^\d+\.0*$', v.strip()):
                return str(int(float(v)))
            # Sadece rakamları içeren float'ları yakala
