# pdf_processor.py - DÜZELTILMIŞ VE TEMİZLENMİŞ
import pdfplumber
import re
...
class PDFProcessor:
        ...
        for i in range(min(3, len(row))):
            ...
            if match:
                ...
                if duplicate_key in self.processed_codes:
                    logging.debug(f"DUPLİKASYON: {product_code} {category}'de zaten işlendi")
                    continue
    ...
    def determine_pdf_type(self, pdf_path: str) -> str:
        ...
        try:
            ...
            logging.warning(f"PDF tipi belirlenemedi, varsayılan kullanılacak: {e}")
            return 'normal'
